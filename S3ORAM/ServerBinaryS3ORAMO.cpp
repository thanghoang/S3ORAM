 ...
 */  
int ServerBinaryS3ORAMO::retrieve(zmq::socket_t& socket)
{
    ...
    for(int i = 0, startIdx = 0; i < numThreads , startIdx < DATA_CHUNKS; i ++, startIdx+=step)
    {
        ...
        pthread_create(&thread_compute[i], NULL, &ServerBinaryS3ORAMO::thread_loadRetrievalData_func, (void*)&loadData_args[i]);
 ...
 * @return 0 if successful
 */  
int ServerBinaryS3ORAMO::evict(zmq::socket_t& socket)
{
    S3ORAM ORAM;
    TYPE_INDEX n_evict;
    ...
    for(int h = 0; h < H+1 ; h++)
    {
		...
		for(int i = 0, startIdx = 0; i < numThreads , startIdx < DATA_CHUNKS; i ++, startIdx+=step)
		{
			...
			pthread_create(&thread_compute[i], NULL, &ServerBinaryS3ORAMO::thread_loadTripletData_func, (void*)&loadData_args[i]);
...
//            recvSocket_args[k] = struct_socket("tcp://*:" + SERVER_PORT[(serverNo)*(NUM_SERVERS)+this->others[k]], NULL, 0, shares_buffer_in[k], BUCKET_SIZE*sizeof(TYPE_DATA)*DATA_CHUNKS, NULL,false);
			...
			pthread_create(&thread_recv[k], NULL, &ServerBinaryS3ORAMO::thread_socket_func, (void*)&recvSocket_args[k]);
		}
		...
		for (int i = 0; i < NUM_SERVERS-1; i++)
		{
			...
			pthread_create(&thread_send[i], NULL, &ServerBinaryS3ORAMO::thread_socket_func, (void*)&sendSocket_args[i]);
		}
		cout<< "	[evict] CREATED!" <<endl;
		//=================================================================================================================
 ...
 * @return 0 if successful
 */  
int ServerBinaryS3ORAMO::multEvictTriplet(zz_p** evictMatrix)
{
 ...
 * 
 */  
void* ServerBinaryS3ORAMO::thread_loadTripletData_func(void* args)
{
    ...
    for(int i = 0; i < 2; i++)
    {
        file_in = NULL;
        if(i==1)
            path = rootPath + to_string(opt->serverNo) + "/" + to_string(opt->destIdx);
